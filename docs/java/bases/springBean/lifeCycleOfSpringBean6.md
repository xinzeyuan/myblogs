...
3. 准备运行的环境(dev，test)，根据环境解析不同的配置文件
  + 检查webEnvironment属性，如果是web程序，那么创建一个StandardServletEnvironment对象，否则创建StandardEnvironment对象
  + 准备运行环境配置，根据环境(dev，test)，加载相关配置文件
  + 利用spring应用执行监听器广播运行环境加载完毕的事件
4. 创建spring应用上下文，如果是web程序，创建web应用上下文实例
5. 准备spring应用上下文
  + 利用spring初始化器，对上下文做初始化操作
  + 利用spring应用执行监听器广播上下文准备好的事件
  + 向beanFactory注册springApplicationArguments和springBootBanner单例bean
  + 向上下文注册启动类的bean，也就是调用SpringApplication.run(Application.class, args);的类
  + 利用spring应用执行监听器广播上下文加载完毕事件
6. 刷新spring应用上下文
  + 为刷新上下文信息做准备。例如清空缓存，初始化属性信息，验证必要的属性等
  + 获取应用上下文的beanFactory
  + 对beanFactory设置相关参数
  ...
  + 调用beanFactory的后置处理器
  + 注册bean的后置处理器，这些后置处理器将在bean的创建过程中被调用
  + 初始化事件广播器(EventMulticaster)，看是否有自定义的事件广播bean(applicationEventMulticaster)，如果有则使用自定义的，否则就用默认的。
  + 初始化其他特殊的bean，比如创建serverlet容器相关的bean
  + 向事件广播器EventMulticaster中添加各种事件监听器
  + 实例化所有单例bean
  + 结束刷新，利用EventMulticaster广播上下文刷新完成事件
